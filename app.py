...
auth =('%s:%s' % (os.environ['CTM_TOKEN'], os.environ['CTM_SECRET']))
print(auth)

app.config.update(dict(
  DEBUG=(os.environ['CTM_ENV'] == 'development'),
  SECRET_KEY='Update to something secure',
  CTM_AUTH=base64.standard_b64encode(auth.encode()).decode(),
...
def get_http_conn():
  if app.config['DEBUG']:
    return http.client.HTTPSConnection(app.config['CTM_HOST'], 443, None, None, socket._GLOBAL_DEFAULT_TIMEOUT, None)
  return http.client.HTTPSConnection(app.config['CTM_HOST'])

def ctm_get(endpoint, query=dict()):
  conn = get_http_conn()
  headers = { 'Authorization' : 'Basic %s' %  app.config['CTM_AUTH'] }
  conn.request("GET","/api/v1/" + endpoint + ".json?" + urllib.parse.urlencode(query,True), headers=headers)
...
def ctm_post(endpoint, post_body=dict()):
  conn = get_http_conn()
  headers = { 'Authorization' : 'Basic %s' %  app.config['CTM_AUTH'] }
  conn.request("POST","/api/v1/" + endpoint + ".json", body=urllib.parse.urlencode(post_body,True), headers=headers)
  res = conn.getresponse()
  print(res.status, res.reason)
  if res.status == 200:
    return json.load(res)
